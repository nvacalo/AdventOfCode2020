...
namespace Puzzles
{
    public static class Day21
    {
        ...
        //public static Dictionary<List<string>, List<string>> _AllergenIngredients = new Dictionary<List<string>, List<string>>();
        public static Dictionary<string, List<string>> _IngredientAllergens = new Dictionary<string, List<string>>();
        ...
        public static void RunDay()
        {
            ...
            foreach (var line in input)
            {
                ...
                f.Ingredients = new List<string>();
                foreach (var part in parts[0].Split(" "))
                {
                    if (string.IsNullOrWhiteSpace(part) == false) f.Ingredients.Add(part);
                }

                f.Allergens = new List<string>();
                foreach (var part in parts[1].Split(","))
                {
                    if (string.IsNullOrWhiteSpace(part) == false)
                    {
                        var allergen = part.Replace(")", "").Replace(" ", "");
                        f.Allergens.Add(allergen);
                    }
                }

                foreach (var ingred in f.Ingredients)
                {
                    if (_IngredientAllergens.ContainsKey(ingred))
                    {
                        _IngredientAllergens[ingred].AddRange(JsonConvert.DeserializeObject<List<string>>(JsonConvert.SerializeObject(f.Allergens)));
                    }
                    else
                    {
                        _IngredientAllergens[ingred] = JsonConvert.DeserializeObject<List<string>>(JsonConvert.SerializeObject(f.Allergens));
                    }
                }

                //var allergens = f.Allergens.OrderBy(a => a).ToList();

                //if (_AllergenIngredients.ContainsKey(allergens))
                //{
                //    _AllergenIngredients[allergens].AddRange(JsonConvert.DeserializeObject<List<string>>(JsonConvert.SerializeObject(f.Ingredients)));
                //}
                //else
                //{
                //    _AllergenIngredients[allergens] = JsonConvert.DeserializeObject<List<string>>(JsonConvert.SerializeObject(f.Ingredients));
                //}

                _Foods.Add(f);
            }

            //foreach (var pair in JsonConvert.DeserializeObject<Dictionary<List<string>, List<string>>>(JsonConvert.SerializeObject(_AllergenIngredients)))
            //{
            //    _AllergenIngredients[pair.Key] = pair.Value.Distinct().ToList();
            //}

            foreach (var pair in JsonConvert.DeserializeObject<Dictionary<string, List<string>>>(JsonConvert.SerializeObject(_IngredientAllergens)))
            {
                _IngredientAllergens[pair.Key] = pair.Value.Distinct().ToList();
            }

            //foreach (var pair in _AllergenIngredients)
            //{
            //    foreach (var ingAl in _IngredientAllergens)
            //    {
            //        if (singlePair.Value.Contains(ingAl.Key) == false)
            //        {
            //            Console.WriteLine($"Remove {singlePair.Key} from {ingAl.Key}");
            //            ingAl.Value.Remove(singlePair.Key);
            //        }
            //    }
            //}

            foreach (var ingr in _IngredientAllergens.Keys)
            {
                List<string> wrongAllergens = new List<string>();

                foreach (var allergen in _IngredientAllergens[ingr])
                {
                    foreach (var food in _Foods.Where(f => f.Allergens.Contains(allergen)))
                    {
                        if (food.Ingredients.Contains(ingr) == false)
                        {
                            wrongAllergens.Add(allergen);
                        }
                    }
                }

                foreach (var wr in wrongAllergens)
                {
                    _IngredientAllergens[ingr].Remove(wr);
                }
            }

            bool changed = true;
            while (_IngredientAllergens.Count(kvp => kvp.Value.Count > 1) > 0 && changed)
            {
                changed = false;
                foreach (var food in _Foods)
                {
                    ...
                    {
                        if (_IngredientAllergens[ing].Count > 1)
                        {
                            string unique = HasUniqueAllergenInFood(ing, food);
                            if (unique != null)
                            {
                                ...
                                foreach (var ingAl in _IngredientAllergens.Where(i => i.Key != ing))
                                {
                                    ingAl.Value.Remove(unique);
                                }

                                _IngredientAllergens[ing] = new List<string>() { unique };
                            }
                        }
                    }
                }
            ...
            foreach (var obj in _IngredientAllergens.OrderBy(ia => ia.Value.Count))
            {
                Console.WriteLine($"{obj.Value.Count} - {obj.Key}");
            }
            ...
            foreach (var food in _Foods)
            {
                foreach (var ing in food.Ingredients)
                {
                    if (_IngredientAllergens[ing].Count == 0)
                    {
                        count++;
                    }
                }
            ...
            string answer = "";
            foreach (var obj in _IngredientAllergens.Where(a => a.Value.Count == 1).OrderBy(ia => ia.Value[0]))
            {
                Console.WriteLine($"{obj.Value[0]} - {obj.Key}");
                answer += $"{obj.Key},";
            }
        ...
        private static string HasUniqueAllergenInFood(string ing, Food food)
        {
            foreach (var al in food.Allergens)
            {
                if (_IngredientAllergens[ing].Contains(al))
                {
                    var flag = true;
                    foreach (var i in food.Ingredients.Where(i => i != ing))
                    {
                        if (_IngredientAllergens[i].Contains(al))
                        {
                            flag = false;
                            break;
                        }
