...
namespace Puzzles
{
    public static class Day18
    {
        public static void RunDay()
        {
            ...
            foreach (var e in input)
            {
                ...
                while (exp.Contains("("))
                {
                    ...
                    long innerRes = Evaluate(exp.Substring(start + 1, close - (start + 1)));

                    exp = $"{exp.Substring(0, start)}{innerRes}{exp.Substring(close + 1, exp.Length - (close + 1))}";
                }

                long res = Evaluate(exp);
        ...
        private static long Evaluate(string exp)
        {
            ...
            long res;
            while (long.TryParse(exp, out res) == false)
            {
                int start = 0;
                int end = 0;
                long? a = null;
                ...
                string s = "";
                long r;
                for (int idx = begin; idx < exp.Length; idx++)
                {
                    if (exp[idx] == '+' || exp[idx] == '*')
                    {
                        if (s != "") break;
                    ...
                    else
                    {
                        ...
                        {
                            if (a == null) start = idx;
                            a = long.Parse($"{a}{exp[idx]}");
                        }
                        else
                        {
                            if (b == null) end = idx;
                            b = long.Parse($"{b}{exp[idx]}");
                        }
                    }
                }
                ...
                if (s == "+")
                {
                    ...
                    var temp = exp;
                    exp = $"{exp.Substring(0, start)}{r}{exp.Substring(end + b.Value.ToString().Length, exp.Length - (end + b.Value.ToString().Length))}";
                    //Console.WriteLine($"{temp}: {exp}");
                    begin = 0;
                }
                else if (exp.Contains("+") == false)
                {
                    r = a.Value * b.Value;
