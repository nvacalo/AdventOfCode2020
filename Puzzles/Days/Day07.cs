...
namespace Puzzles
{
    ...
    {
        static Dictionary<string, List<Container>> _BagsRules = new Dictionary<string, List<Container>>();
        ...
        public static void RunDay()
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    var containings = line.Split("contain ")[1];
                    var bags = containings.Split(", ");
                    ...
                    foreach (var bag in bags)
                    {
                        ...
                        var words2 = bag.Split(' ');

                        cont.Size = Int32.Parse(words2[0]);
                        cont.Color = $"{words2[1]} {words2[2]}";

                        containers.Add(cont);
                    }
                }

                _BagsRules[color] = containers;
            }

            int count = 0;
            //foreach (var bagRule in _BagsRules)
            //{
            //    if (CanContain(bagRule.Value, "shiny gold"))
            //    {
            //        count++;
            //    }
            //}

            var goldRules = _BagsRules["shiny gold"];
            foreach (var rule in goldRules)
            {
                count = count + NumberOfBags(rule);
            }

            Console.WriteLine(count);
        }

        private static int NumberOfBags(Container rule)
        {
            int count = rule.Size;

            foreach (var r in _BagsRules[rule.Color])
            {
                count += rule.Size * NumberOfBags(r);
            }

            return count;
        }

        private static bool CanContain(List<Container> containers, string color)
        {
            bool contains = false;
            ...
            foreach (var container in containers)
            {
                ...
                else if (_BagsRules.ContainsKey(container.Color))
                {
                    contains = CanContain(_BagsRules[container.Color], color);
                }
