...
namespace Puzzles
{
    public static class Day16
    {
        ...
        public static List<Ticket> _NearByTickets = new List<Ticket>();
        public static Ticket _MyTicket = new Ticket();

        public class Field
        {
            public string Name { get; set; }
            public List<int> ValidValues1 { get; set; }
            public List<int> ValidValues2 { get; set; }
            public List<int> CandidatePositions { get; set; }
        }
        ...
        {
            //var fields = GetClassTest();
            //var ticket = GetTicketTest();
            //var nearby = GetNearByTicketsTest();

            var fields = GetClass();
            var ticket = GetTicket();
            var nearby = GetNearByTickets();

            int positions = ticket.Split(",").Length;

            _MyTicket.Values = new Dictionary<int, int>();
            int idx = 1;
            foreach (var num in ticket.Split(","))
            {
                int number = int.Parse(num);
                _MyTicket.Values[idx++] = number;
            }
            ...
            {
                field = new Field();
                field.CandidatePositions = new List<int>();
                for (int i = 1; i <= positions; i++)
                {
                    field.CandidatePositions.Add(i);
                }

                var name = f.Split(": ")[0];
                var ranges = f.Split(": ")[1];

                field.Name = name;
                var firstRange = ranges.Split(" or ")[0];
                var secondRange = ranges.Split(" or ")[1];

                int rangeFor = int.Parse(firstRange.Split("-")[0]);
                int rangeTo = int.Parse(firstRange.Split("-")[1]);

                field.ValidValues1 = new List<int>();
                for (int i = rangeFor; i <= rangeTo; i++)
                {
                    field.ValidValues1.Add(i);
                }

                rangeFor = int.Parse(secondRange.Split("-")[0]);
                rangeTo = int.Parse(secondRange.Split("-")[1]);

                field.ValidValues2 = new List<int>();
                for (int i = rangeFor; i <= rangeTo; i++)
                {
                    field.ValidValues2.Add(i);
                }
            ...
            bool valid;
            foreach (var nrt in nearby)
            {
                valid = true;
                Ticket nearbyTicket = new Ticket();
                nearbyTicket.Values = new Dictionary<int, int>();

                int i = 1;
                foreach (var num in nrt.Split(","))
                {
                    int number = int.Parse(num);
                    ...
                    nearbyTicket.Values[i++] = number;
                }

                if (valid) _NearByTickets.Add(nearbyTicket);
            }

            foreach (var fld in _Fields)
            {
                foreach (var nrt in _NearByTickets)
                {
                    foreach (var v in nrt.Values)
                    {
                        if (fld.ValidValues1.Contains(v.Value) == false && fld.ValidValues2.Contains(v.Value) == false)
                        {
                            fld.CandidatePositions.Remove(v.Key);
                        }
                    }
                }
            }
            ...
            {
                Console.Clear();
                foreach (var fld in _Fields.Where(f => f.CandidatePositions.Count > 1))
                {
                    string text = "";
                    foreach (var x in fld.CandidatePositions)
                    {
                        text += x.ToString() + ",";
                    }

                    Console.WriteLine($"{fld.CandidatePositions.Count} - {fld.Name} - {text}");
                }
                Console.WriteLine();
                ...
                {
                    int uniquePos = 0;
                    foreach (var pos in fld.CandidatePositions)
                    {
                        var onlyMe = true;
                        foreach (var restFld in _Fields.Where(f => f.Name != fld.Name))
                        {
                            foreach (var othPos in restFld.CandidatePositions)
                            {
                                if (othPos == pos)
                                {
                                    onlyMe = false;
                    ...
                    if (uniquePos > 0)
                    {
                        fld.CandidatePositions = new List<int>() { uniquePos };
                        foreach (var restFld in _Fields.Where(f => f.Name != fld.Name))
                        {
                            restFld.CandidatePositions.Remove(uniquePos);
                        }
                ...
                if (noChange)
                {
                    ...
                    var sel = _Fields.Where(f => f.Name == name).FirstOrDefault();
                    foreach (var restFld in _Fields.Where(f => f.Name != sel.Name))
                    {
                        restFld.CandidatePositions.Remove(n);
                    }

                    sel.CandidatePositions = new List<int>() { n };

                    //if (memoryFields == null)
                    //{
                    //    memoryFields = JsonConvert.DeserializeObject<List<Field>>(JsonConvert.SerializeObject(_Fields));
                    //}
                    //else
                    //{
                    //    _Fields = memoryFields;
                    //    memoryPositionIndex = 0;
                    //}

                    //var sel = _Fields.Where(f => f.CandidatePositions.Count > 1).OrderBy(f => f.CandidatePositions.Count).First();

                    //int toRemove = sel.CandidatePositions[memoryPositionIndex];
                    //foreach (var restFld in _Fields.Where(f => f.Name != sel.Name))
                    //{
                    //    restFld.CandidatePositions.Remove(toRemove);
                    //}

                    //sel.CandidatePositions = new List<int>() { toRemove };
                }

                foreach (var fld in _Fields)
                {
                    if (fld.CandidatePositions.Count > 1)
                    {
                        again = true;
                        break;
                    }
            ...
            foreach (var fld in _Fields)
            {
                if (fld.Name.Contains("departure"))
                {
                    Console.WriteLine(_MyTicket.Values[fld.CandidatePositions[0]]);
                    mul *= _MyTicket.Values[fld.CandidatePositions[0]];
                }
            }
        ...
        private static bool NumberIsInvalid(int num)
        {
            foreach (var field in _Fields)
            {
                foreach (var v1 in field.ValidValues1)
                {
                    if (v1 == num)
                    {
                        return false;
                ...
                foreach (var v2 in field.ValidValues2)
                {
                    if (v2 == num)
                    {
                        return false;
        ...
        private static List<string> GetClassTest()
        {
        ...
        private static string GetTicketTest()
        {
            return "7,1,14";
        }

        private static List<string> GetNearByTicketsTest()
        {
            return new List<string>()
            {
                "7,3,47",
        ...
        private static List<string> GetClass()
        {
            return new List<string>()
            {
        ...
        {
            return "79,149,97,163,59,151,101,89,173,139,167,61,73,71,137,53,83,157,131,67";
        }

        private static List<string> GetNearByTickets()
        {
            return new List<string>()
            {
