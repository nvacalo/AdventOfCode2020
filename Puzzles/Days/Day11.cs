...
namespace Puzzles
{
    public class Seat1
    {
        ...
        public int NumberOfOccupiedNeighbors(Dictionary<int, Seat1> seats)
        {
            int count = 0;
    ...
    {
        public static Dictionary<int, Seat1> seats = new Dictionary<int, Seat1>();
        ...
        public static void RunDay()
        {
            ...
            {
                Seat1 seat = kvp.Value;
                seat.Neighbors = new List<int>();

                seat.Neighbors.Add(seats.Values.Where(s => s.X == seat.X - 1 && s.Y == seat.Y - 1).FirstOrDefault()?.Id ?? -1);
                seat.Neighbors.Add(seats.Values.Where(s => s.X == seat.X && s.Y == seat.Y - 1).FirstOrDefault()?.Id ?? -1);
                seat.Neighbors.Add(seats.Values.Where(s => s.X == seat.X + 1 && s.Y == seat.Y - 1).FirstOrDefault()?.Id ?? -1);
                seat.Neighbors.Add(seats.Values.Where(s => s.X == seat.X - 1 && s.Y == seat.Y).FirstOrDefault()?.Id ?? -1);
                seat.Neighbors.Add(seats.Values.Where(s => s.X == seat.X + 1 && s.Y == seat.Y).FirstOrDefault()?.Id ?? -1);
                seat.Neighbors.Add(seats.Values.Where(s => s.X == seat.X - 1 && s.Y == seat.Y + 1).FirstOrDefault()?.Id ?? -1);
                seat.Neighbors.Add(seats.Values.Where(s => s.X == seat.X && s.Y == seat.Y + 1).FirstOrDefault()?.Id ?? -1);
                seat.Neighbors.Add(seats.Values.Where(s => s.X == seat.X + 1 && s.Y == seat.Y + 1).FirstOrDefault()?.Id ?? -1);

                seat.Neighbors.RemoveAll(s => s == -1);
            }
            ...
            {
                cont = false;
                var previousSeats = JsonConvert.DeserializeObject<Dictionary<int, Seat1>>(JsonConvert.SerializeObject(seats));
                ...
                foreach (var kvp in seats)
                {
                    if (kvp.Value.isFloor() == false)
                    {
                        ...
                        else if (kvp.Value.isOccupied() && kvp.Value.NumberOfOccupiedNeighbors(previousSeats) > 3)
                        {
                            cont = true;
                            kvp.Value.Empty();
                        }
